...
namespace Int\ExtenedFilter\Model\Import;
 ...
 * @since 100.0.2
 */
class Products extends \Magento\CatalogImportExport\Model\Import\Product
{
    const CONFIG_KEY_PRODUCT_TYPES = 'global/importexport/import_product_types';
    const CONFIG_IMPORT_ALLOW_UPDATION = 'int_product/import/allow_updation';
    ...
    protected function _saveProducts()
    {
        ...
        while ($bunch = $this->_dataSourceModel->getNextBunch()) {
            ...
            foreach ($bunch as $rowNum => $rowData) {
                ...
                $rowSkus = $rowData[self::COL_SKU];
                $products = $this->retrieveProductBySku($rowSkus);
                if(isset($products)){
                    $stockItemDos = $this->stockRegistry->getStockItem($products->getEntityId());
                    $this->exsistingStockData[$products->getSku()] = $stockItemDos->getData();
                }
        ...
        while ($bunch = $this->_dataSourceModel->getNextBunch()) {
            ...
            foreach ($bunch as $rowNum => $rowData) {
                ...
                if (!isset($stockData[$sku])) {
                    $stockData[$sku] = $this->getStockdataWithPreviousData($sku, $row);
                }
            }
            // Insert rows
            if (!empty($stockData)) {
    ...
    public function getStockdataWithPreviousData($sku, $row)
    {
        if($this->getImportConfig() !== true){
            return $row;
        }
        ...
        if (isset($this->exsistingStockData[$sku]['qty'])) {
            $updateQty = $row['qty'] + $this->exsistingStockData[$sku]['qty'];
            $row["qty"] = $updateQty;
        }
    ...
    protected function getImportConfig()
    {
        return (bool) $this->scopeConfig->getValue(
            self::CONFIG_IMPORT_ALLOW_UPDATION,
