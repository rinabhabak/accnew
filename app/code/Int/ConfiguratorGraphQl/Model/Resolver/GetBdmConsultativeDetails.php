...
/**
 * Class GetBdmConsultativeDetails
 * @package Int\ConfiguratorGraphQl\Model\Resolver
 */
class GetBdmConsultativeDetails implements ResolverInterface
{
    ...
    public function resolve(
		...
		if(isset($args['input']['status']) && $args['input']['status']!='') {
			...
			$collection->getSelect()->join(
				...
				"config.status = ".$status." AND is_consulatative_sale=1 AND main_table.assigned_to=".$bdmId
			);
		}
		else {
			$collection->getSelect()->join(
				...
				"is_consulatative_sale=1 AND main_table.assigned_to=".$bdmId
			);
