...
/**
 * Class GetBdmPastConsultativeDetails
 * @package Int\ConfiguratorGraphQl\Model\Resolver
 */
class GetBdmPastConsultativeDetails implements ResolverInterface
{
    ...
    public function resolve(
		...
		if(!isset($args['input']['bdm_id']) || $args['input']['bdm_id'] == '') {
		...
		$bdmId = $args['input']['bdm_id'];
		$collection = $this->_bdmManagerCollection->create();
		if(isset($args['input']['status']) && $args['input']['status']!='') {
		   ...
		   $collection->getSelect()->join(
				...
				"config.status = ".$status." AND config.created_at <= '".$past_date."' AND is_consulatative_sale=1 AND main_table.assigned_to=".$bdmId
			);
			 ...
			 $collection->getSelect()->join(
				...
				"is_consulatative_sale=1 AND config.created_at <= '".$past_date."' AND main_table.assigned_to=".$bdmId
			);
