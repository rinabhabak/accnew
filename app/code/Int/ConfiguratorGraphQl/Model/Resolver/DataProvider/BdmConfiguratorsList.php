...
class BdmConfiguratorsList
{
    ...
    public function getGetConfiguratorList($bdmId,$status)
    {
        ...
        if($status!=='' && $status!==NULL){
            $_configurators->getSelect()->where("bdm_managers_assigned.assigned_to=".$bdmId." AND main_table.status=".$status);
        }else{
            $_configurators->getSelect()->where("bdm_managers_assigned.assigned_to=".$bdmId);
        }
       ...
       // }
        ...
        foreach ($_configurators as $key => $_configurator)
        {
			...
			$configuration[$key]['config_updated_at'] = $_configurator->getUpdatedAt();
