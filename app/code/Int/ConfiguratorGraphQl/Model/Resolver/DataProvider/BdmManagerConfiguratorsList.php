...
class BdmManagerConfiguratorsList
{
    /**
    ...
    public function getGetConfiguratorList($status)
    {
        $configuration = [];
        ...
        if($status != NUll && $status !=''){
            ...
            $collection->addFieldToFilter('bds_status', ['in' => $statusArray]);
            $collection->addFieldToFilter('status', ['eq' => \Int\Configurator\Model\Status::STATUS_COMPLETE]);
            //$collection->addFieldToFilter('is_consulatative_sale', 0);
        }
        ...
        foreach ($collection as $key => $configurator)
        {
            ...
            $configuration[$key]['status'] = $this->_configuratorStatus->getOptionText($configurator->getStatus())?$this->_configuratorStatus->getOptionText($configurator->getStatus()):'NA';
			
			$configuration[$key]['bds_status'] = $configurator->getBdsStatus();
			$configuration[$key]['bds_completed_date'] = $configurator->getBdsCompletedDate();
            ...
            $configuration[$key]['creation_date'] = ($configurator->getCreatedAt())?$configurator->getCreatedAt():'NA';
			$configuration[$key]['config_created_at'] = ($configurator->getCreatedAt())?$configurator->getCreatedAt():'NA';
			$configuration[$key]['config_updated_at'] = ($configurator->getUpdatedAt())?$configurator->getUpdatedAt():'NA';
            $configuration[$key]['customer_name'] = ($configurator->getCustomerId())?$this->getCustomerName($configurator->getCustomerId()):'NA';
            if($bdmManager->getId()){
                $configuration[$key]['assigned_bdm_id'] = $bdmManager->getAssignedTo();
                $configuration[$key]['assigned_bdm_date'] = $bdmManager->getUpdatedAt();
