...
class Edit extends \Magento\Backend\App\Action
{
    ...
    public function execute()
    {
        ...
        if ($id) {
            $model->load($id);
            if (!$model->getId()) {
                $this->messageManager->addError(__('This bdm no longer exists.'));
        ...
        $this->_coreRegistry->register('bdmmanagement', $model);

        // 5. Build edit form
		/** @var \Magento\Backend\Model\View\Result\Page $resultPage */
        $resultPage = $this->_initAction();
        $resultPage->addBreadcrumb(
            $id ? __('Edit Bdm') : __('New Bdm'),
            $id ? __('Edit Bdm') : __('New Bdm')
        );
        $resultPage->getConfig()->getTitle()->prepend(__('Bdm anagement'));
        $resultPage->getConfig()->getTitle()
            ->prepend($model->getId() ? $model->getTitle() : __('New Bdm'));
        return $resultPage;
    }
}
