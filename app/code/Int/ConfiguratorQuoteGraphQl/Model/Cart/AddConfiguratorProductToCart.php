...
namespace Int\ConfiguratorQuoteGraphQl\Model\Cart;
 ...
 */
class AddConfiguratorProductToCart
{
    /**
    ...
    public function execute(Quote $cart, array $cartItemData): void
    {
        ...
        try {
            $cartItem = $cart->addProduct($product, $this->buyRequestBuilder->build($cartItemData));
            
            $additional_options = isset($cartItemData['additional_options'])?$cartItemData['additional_options']:array();
            if($additional_options){
                $additionalOptions = array();
                if ($additionalOption = $cartItem->getOptionByCode('additional_options')){
                    $additionalOptions = (array) unserialize($additionalOption->getValue());
                }
                
                 
                foreach($additional_options as $additional_option){
                    $additionalOptions[] = $additional_option;
                }
                
                if(count($additionalOptions) > 0){
                    $cartItem->addOption(array(
        ...
        if (is_string($cartItem)) {
            throw new GraphQlInputException(__($cartItem));
        }
    }
