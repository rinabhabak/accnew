define([
		...
		function startDictation(class_name_mic_on,class_name_mic_off)
		{
			if (window.hasOwnProperty('webkitSpeechRecognition'))
			{
				...
				{
					document.getElementById(window.searchbyvoice_search_id).value = e.results[0][0].transcript;//Array.from(e.results).map(result => result[0]).map(result => result.transcript).join('');
					recognition.stop();
