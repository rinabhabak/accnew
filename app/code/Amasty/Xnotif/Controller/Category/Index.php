...
class Index extends \Magento\Framework\App\Action\Action
{
        ...
        if ($productCollection && $productCollection->getSize()) {
            foreach ($productCollection as $product) {
                ...
                foreach ($this->configurableBlock->getAllowProducts() as $simpleProduct) {
                    if ($this->isProductSaleable($simpleProduct)) {
                        continue;
                    }
     ...
     * @return bool
     */
    protected function isProductSaleable(\Magento\Catalog\Model\Product $product)
    {
        $stockStatus = $this->stockRegistry->getStockStatusBySku(
            $product->getSku(),
            $this->storeManager->getWebsite()->getId()
        );
        $verifyStock = $this->helper->verifyStock($stockStatus);

        return $stockStatus->getStockStatus() && $verifyStock;
    }
