...
class CustomerRequisitionListsTest extends GraphQlAbstract
{
    ...
    public function testCustomerRequisitionList(): void
    {
        ...
        $query = $this->getQuery($requisitionListId);
        ...
        $this->assertEquals(base64_encode((string)$requisitionListId), $list['uid']);
        $this->assertNotEmpty($list['name']);
    ...
    public function testGuestCannotGetRequisitionList(): void
    {
        ...
        $this->graphQlQuery($this->getQuery(null));
    }
     ...
     * @return string
     */
    private function getQuery(?int $requisitionListId): string
    {
        return <<<QUERY
query {
  customer {
